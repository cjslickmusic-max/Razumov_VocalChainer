...
**Иерархия экрана и смысл блоков** (макросы как hero, две зоны, микрофон в инспекторе, масштаб) — в **`docs/PRODUCT_UX_CONCEPT.md`**; этот раздел про токены и визуальные правила.

1. **Светлая «стеклянная» база** (текущее направление JUCE-редактора) — см. `Source/ui/DesignTokens.h` и `tokens.json`; контраст текста и контролов как в DAW (см. `.cursor/rules/ui-strings-ascii.mdc` для строк).
