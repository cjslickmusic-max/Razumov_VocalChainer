...
void drawKnobSoftShadowStack(juce::Graphics& g, juce::Point<float> centre, float radius, float alphaMul)
{
    ...
    /** Soft tail under knob only: light scatter toward value row (avoid heavy box on digits). */
    {
        const float dropS = (radius + 6.5f) / 52.0f * 1.12f;
        ...
        const float dy = centre.y + 14.0f - kAmbientRefY * dropS * squash;
        g.setColour(juce::Colour(0x42000000).withMultipliedAlpha(0.22f * alphaMul));
        g.drawImage(maps.ambient,
                    (int) std::floor(dx),
                    (int) std::floor(dy),
                    (int) std::ceil(w),
    ...
    {
        const float dropS = (radius + 8.0f) / 52.0f * 1.22f;
        ...
        const float dy = centre.y + 22.0f - kContactRefY * dropS * 0.82f;
        g.setColour(juce::Colour(0x28000000).withMultipliedAlpha(0.18f * alphaMul));
        g.drawImage(maps.contact,
                    (int) std::floor(dx),
                    (int) std::floor(dy),
                    (int) std::ceil(w),
