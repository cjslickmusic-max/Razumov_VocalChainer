...
**Версия:** `project(... VERSION x.y.z)` и `VERSION` внутри `juce_add_plugin` — держать синхронно в `CMakeLists.txt` (сейчас **0.9.23**: `COPY_PLUGIN_AFTER_BUILD` OFF — без копии в `~/Library/...`; VST3 в `Artifacts/` + `build/.../artefacts/`; цепочка в UI по дереву графа + fork/merge; phase align на split-ветках + PDC; макросы x8 с назначением на параметр; per-slot модули + chain/mic profile; спектральный компрессор: сайдчейн-полоса Q + A/R, UI полоса/порог/уровень).
